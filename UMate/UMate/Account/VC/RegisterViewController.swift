...
class RegisterViewController: UIViewController {
    ...
    /// 노티피케이션 데이타를 저장하기 위해서  만든 빈 문자열 속성
    var saveText = ""
    ...
    /// 메뉴 인스턴스를 클로저로 초기화
    /// 실제 Date에 맞춰서 14년 간격으로  학번을 문자열 배열로 리턴 합니다.
    let menu: DropDown? = {
        let menu = DropDown()
    ...
    /// 화면을 Dismiss 합니다.
    /// - Parameter sender: UIButton으로 전달되서 이전화면으로 갑니다.
    ...
    /// - Parameter sender: UIButton에 전달되서 선택된 셀의 값을 UserDefaults에 저장합니다.
    @IBAction func makeEnterenceOfYearData(_ sender: UIButton) {
        // 높이가 적절 하다면 드롭다운을 show 합니다.
        menu?.show()
        // 사용자가 셀을 선택시 enterenceYearLabel에 선택한 셀의 item을 전달합니다.
    ...
    /// - Parameter sender: nextButton 로 전달되면 조건에 맞는 경우  다음 화면으로 이동합니다.
