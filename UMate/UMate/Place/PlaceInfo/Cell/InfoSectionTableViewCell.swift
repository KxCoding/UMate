...
class InfoSectionTableViewCell: UITableViewCell {
    ...
    /// 북마크 상태를 토글합니다.
    ///
    /// 버튼의 선택 상태를 전환해 버튼 이미지를 변경한 후, 사용자 데이터에 가게가 포함되어 있으면 삭제하고 없으면 추가합니다.
    /// - Parameter sender: 버튼
    ...
    ///
    /// 화면에서 전달된 가게 객체에 따라 대상 가게를 저장하고 뷰의 데이터를 초기화합니다.
    /// 가게의 인스타그램 아이디나 웹페이지 정보가 없으면 버튼을 표시하지 않고, 북마크 상태에 따라 버튼 상태를 변경합니다.
